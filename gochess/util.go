...
func Lazi(chessManual *ChessManual, dropPoint DropPoint) *[]DropPoint {
	...
	if !chessManual.CheckFowardPoint1(dropPoint) {
		fmt.Println("two continuation step are the same!")
		return nil
	}

	f, dropPoints := isLaziInvalid(chessManual, dropPoint)
	if f {
		if chessManual.Ko.State == dropPoint.State {
			fmt.Println("----------Ko state == droppoint state----")
			chessManual.Ko.State = PointStateBlank
		}
...
func isLaziInvalid(chessManual *ChessManual, dropPoint DropPoint) (bool, []DropPoint) {
	...
	if len(kill) == 1 {
		...
		*/
		if chessboard.CountQi(dropPoint) == 0 {
			if chessManual.CheckKoPoint(dropPoint) {
				fmt.Println("invalid drop:", dropPoint, chessManual.Ko)
				return false, nil
			}
			chessManual.Ko = kill[0]
		}
		return true, kill
	}
